package dev.bildungsroman.tetrominauts.ui.stars

import android.content.Context
import android.graphics.Canvas
import android.util.AttributeSet
import android.view.View
import dev.bildungsroman.tetrominauts.R
import dev.bildungsroman.tetrominauts.ui.stars.entities.InterstellarFactory
import dev.bildungsroman.tetrominauts.ui.stars.entities.Star
import dev.bildungsroman.tetrominauts.ui.stars.entities.StarConstraints
import dev.bildungsroman.tetrominauts.ui.stars.entities.meteor.Meteorite
 ...
 *
 * Used in Wakey - Cute & Gentle Alarm Clock for Android: http://bit.ly/2uI8pgL Check out the
 * article on Medium: http://bit.ly/2NlFJBW Or see what it looks like on YouTube:
 * https://www.youtube.com/watch?v=v1-228CkoQc
 *
...
constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    init {
        ...
        meteoritesEnabled =
            array.getBoolean(R.styleable.AnimatedStarsView_starsView_meteoritesEnabled, false)
        meteoritesInterval =
            array.getInt(R.styleable.AnimatedStarsView_starsView_meteoritesInterval, 5000)

        val starColorsArrayId =
            array.getResourceId(R.styleable.AnimatedStarsView_starsView_starColors, 0)
        val meteoritesColorsArrayId =
                        ...
                        postDelayed(
                            {
                                meteorite =
                                    Meteorite(
                                        ...
                                        x = viewWidth,
                                        y =
                                            Math.round(Math.random() * (viewHeight * 2 / 3))
                                                .toInt(),
                                        color =
                                            meteoritesColors[random.nextInt(meteoritesColors.size)],
        ...
        synchronized(stars) {
            stars =
                MutableList(starCount) {
                    val starCompleteListener =
                        object : BaseStar.StarCompleteListener {
